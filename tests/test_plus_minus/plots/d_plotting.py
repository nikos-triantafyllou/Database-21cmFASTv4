...
def plot_lc(lc, lc_redshifts, title = 'lightcone'):
    '''
    Plot a lightcone
    ...
    plt.imshow(lightcone[:,123,:],cmap='EoR',vmin = -150,vmax=30, extent=[0,2553,0,300])
    ax.set_aspect(1.5)
...
def get_mean_std_stuff(x,y, bins=100):
    fig, ax = plt.subplots()
    x =np.log10(x)
    y = np.log10(y)
    print(x, flush=True)
    ...
    half_step = (hist[1][1]-hist[1][0])/2
    xarr = hist[1][:-1] + half_step
    ymean=[]
    yerr=[]
    for i in range(len(hist[0])):
        positions = np.where((x > hist[1][i]) & (x < hist[1][i+1]))[0]
        ydata=y[positions]
        ymean.append(np.mean(ydata))
        yerr.append(np.std(ydata, ddof=1))
    ymean=np.array(ymean)
    yerr=np.array(yerr)

    return xarr, ymean, yerr