...
print(f'Running with counter_{inputs.counter}')
...
print(f'Running plus minus:{parameter_dict_keys[counter]} with values: {varying_params}', flush=True)
...
astro_params = {
    ...
    "F_ESC7_MINI": None, # Default is -2.0 but I put it at None so that it can scale together at first with F_ESC10---------
    "ALPHA_ESC": -0.5,   # ----------------------------------database---------------------------------------
    "M_TURN": 5,       # Default is None ----------------------------------database---------------------------------------
    "R_BUBBLE_MAX": None,
...
cache_path = f'/leonardo_scratch/large/userexternal/ntriant1/database/_cache6/_pm_{parameter_dict_keys[counter]}_cache/'             
...
#     max_redshift = 6.0,
    ...
    global_quantities=("brightness_temp", 'xH_box', 'Tk_box', "temp_kinetic_all_gas"),
#     write=False
)
...
savedir = '/leonardo_work/CNHPC_1497299/ntriantafyllou/database/database3_venv/test_plus_minus/save6/'
filename = savedir+f'pm_{parameter_dict_keys[counter]}_r{random_seed}.h5'    
...
with h5py.File(filename, 'w') as hdf:
    ...
    data_UV_LF_0 = group_coeval_data.create_dataset('UV_LF_0',              data= LF_arr[0])
    data_UV_LF_0.attrs['z_uv'] =  z_uv
    
    data_UV_LF_1 = group_coeval_data.create_dataset('UV_LF_1',              data= LF_arr[1])
    data_UV_LF_1.attrs['z_uv'] =  z_uv
    
    data_UV_LF_2 = group_coeval_data.create_dataset('UV_LF_2',              data= LF_arr[2])
    data_UV_LF_2.attrs['z_uv'] =  z_uv
