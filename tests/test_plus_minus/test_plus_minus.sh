...
#SBATCH --ntasks-per-node=105        # tasks/processes/cores per node max 112 for DCGP and 32 for Booster
...
CORES=15      # Number of cores pre simulation
...
for counter in $COUNTER_RANGE; do
    ...
    /leonardo_work/CNHPC_1497299/ntriantafyllou/database/database3_venv/venv3/bin/python /leonardo_work/CNHPC_1497299/ntriantafyllou/database/database3_venv/test_plus_minus/test_plus_minus.py \
    --counter $counter&
