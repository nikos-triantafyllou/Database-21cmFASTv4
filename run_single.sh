...
#SBATCH --mem=35GB                   # memory per simulation
#SBATCH --time=3:00:00              # time limits: 3 hours
...
NUMBER_OF_SIMS=1  #SHOULD BE THE SAME NUMBER AS THE NUMBER OF SIMS IN THE SLURM COMMANDS ABOVE
...
srun --output="${LOG_DIR}/out_id_${ARGS[$SLURM_ARRAY_TASK_ID-1]}.out" --error="${LOG_DIR}/err_id_${ARGS[$SLURM_ARRAY_TASK_ID-1]}.err" /leonardo_work/CNHPC_1497299/ntriantafyllou/database/database3_venv/venv3116/bin/python /leonardo_work/CNHPC_1497299/ntriantafyllou/database/database3_venv/CREATE_DATABASE/run_single.py \
