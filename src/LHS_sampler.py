...
sampler = qmc.LatinHypercube(d=10, seed=23)
...
all_runs = []
for i, s in enumerate(sample_scaled):
# for s in sample_scaled:
    sample_dict={}
    sample_dict['random_seed'] = i+1
    for j in range(len(param_names)):
        sample_dict[param_names[j]] = s[j]
    all_runs.append(sample_dict)

# Save the results to a JSON file
output_file = 'LHS_SAMPLES.json'
with open(output_file, 'w') as f:
    json.dump({(i+1): run for i, run in enumerate(all_runs)}, f, indent=4)
